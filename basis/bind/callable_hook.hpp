...
//
//  private:
//  ::basis::CallableHookInvoker<double(double)> weakInvoker_;
// };
//
// class MyClassImpl {
//   public:
...
class GlobalCallableHooksRegistry {
 public:
  ...
  // stores `UnownedRef` to function with specific signature
...
#define DEFINE_CALLABLE_HOOK_WITH_PRIORITY(func, impl, priority) \
  ::basis::CallableHookRegistrationData(FROM_HERE, #func, impl, priority)
...
#define CALLABLE_HOOK_INVOKER(func) \
  ::basis::CallableHookInvoker<decltype(func)>(#func)
