  ...
  FakeLockWithChecks<LockPolicyType, R(Args...)>
{
 public:
  ...
  void removeCheck(const CallbackId& guid)
  {
    ...
    if constexpr (LockPolicyType == basis::LockPolicy::FakeMutex)
    {
      DFAKE_SCOPED_RECURSIVE_LOCK(debug_thread_collision_warner_);
      task();
    }
    else if constexpr (LockPolicyType == basis::LockPolicy::Mutex)
    {
...
//      ::basis::ScopedFakeLockWithChecks<LockPolicy::Mutex, basis::FakeLockPolicySkip, FakeLockRunType>
...
//    /// \note It is not real lock, only annotated as lock.
//    /// It just calls callback on scope entry AND exit.
//    ::basis::FakeLockWithChecks<FakeLockRunType> fakeLockToSequence_{
//        ::base::BindRepeating(
...
//  };
template <
  basis::LockPolicy LockPolicyType
  , typename FakeLockPolicyType
  , typename FakeLockCheckType
  , typename R
  , typename... Args
...
#define CREATE_FAKE_THREAD_GUARD(Name) \
  ::basis::FakeLockWithChecks<basis::LockPolicy::Mutex, bool()> \
    Name { \
...
#define CREATE_DYNAMIC_THREAD_GUARD_WITH_CHECK(Name, Callback) \
  ::basis::FakeLockWithChecks<basis::LockPolicy::Mutex, bool()> Name{ \
    ::basis::VerifyNothing::Repeatedly())}
...
#define FAKE_CUSTOM_THREAD_GUARD(Name, Policy, Scope) \
  ::basis::ScopedFakeLockWithChecks<\
    basis::LockPolicy::Mutex \
    , Policy\
    , Scope\
    , bool()\
  > \
