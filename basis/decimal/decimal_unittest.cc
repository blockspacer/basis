...
TEST_F(FixedPointTest, MiscMath)
{
  ...
  EXPECT_EQ(basis::ceil<int32_t>(num), 6);
  EXPECT_EQ(basis::floor<int32_t>(num), 6);
  EXPECT_EQ(basis::abs<int32_t>(num), 6);

  num /= decimal32(2.5, scale_type{-1});
  EXPECT_EQ(static_cast<std::string>(num), "4.8");
  EXPECT_EQ(static_cast<double>(num), 4.8);
  EXPECT_EQ(basis::ceil<int32_t>(num), 4);
  EXPECT_EQ(basis::floor<int32_t>(num), 4);
  EXPECT_EQ(basis::abs<int32_t>(num), 4);

  num *= decimal32(1.234, scale_type{-3});
  EXPECT_EQ(static_cast<std::string>(num), "5.9232");
  EXPECT_EQ(static_cast<double>(num), 5.9232);
  EXPECT_EQ(basis::ceil<int32_t>(num), 5);
  EXPECT_EQ(basis::floor<int32_t>(num), 5);
  EXPECT_EQ(basis::abs<int32_t>(num), 5);

  num /= decimal32(2, scale_type{0});
  EXPECT_EQ(static_cast<std::string>(num), "2.9616");
  EXPECT_EQ(static_cast<double>(num), 2.9616);
  EXPECT_EQ(basis::ceil<int32_t>(num), 2);
  EXPECT_EQ(basis::floor<int32_t>(num), 2);
  EXPECT_EQ(basis::abs<int32_t>(num), 2);
