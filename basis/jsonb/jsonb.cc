...
basis::Status Jsonb::ToJsonbInternal(const base::Value& document, std::string* jsonb) {
  if (document.is_dict()) {
...
basis::Status Jsonb::ToJsonbProcessObject(const base::Value& document,
                                           std::string* jsonb) {
  ...
  if (data_begin_offset != metadata_offset) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "Couldn't process entire data for json object");
  }

  RETURN_OK();
}

basis::Status Jsonb::ToJsonbProcessArray(const base::Value& document,
  ...
  if (data_begin_offset != metadata_offset) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "Couldn't process entire data for json array");
  }
  RETURN_OK();
}
...
basis::Status Jsonb::ProcessJsonValueAndMetadata(const base::Value& value,
  ...
  switch (value.type()) {
    ...
    case base::Value::Type::BINARY:
      NOTIMPLEMENTED();
      return ::basis::Status(
        FROM_HERE, ::basis::error::INTERNAL, "Numeric type is not supported");
...
Status Jsonb::FromJsonbInternal(const base::span<const char>& jsonb, base::Value* document) {
  ...
  if ((jsonb_header & kJBObject) == kJBObject) {
    ...
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "Invalid json type!");
  }
  RETURN_OK();
}
...
Status Jsonb::GetObjectValue(size_t index, const base::span<const char>& jsonb, size_t metadata_begin_offset,
  ...
  if (value_index >= jsonb.size()) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "value index in jsonb out of bounds");
  }
  ...
  if (value_offset + value_length > jsonb.size()) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "json value data out of bounds in serialized jsonb");
  }
...
Status Jsonb::GetObjectKey(size_t index, const base::span<const char>& jsonb, size_t metadata_begin_offset,
  ...
  if (key_index >= data_begin_offset) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "key index in jsonb out of bounds");
  }
  ...
  if (key_offset + key_length > jsonb.size()) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "json key data out of bounds in serialized jsonb");
  }
...
Status Jsonb::GetArrayElement(size_t index, const base::span<const char>& jsonb,
  ...
  if (value_index >= jsonb.size()) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "value index out of bounds");
  }
  ...
  if (value_offset + value_length > jsonb.size()) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "json value out of bounds of serialized jsonb");
  }
  *result = base::span<const char>(jsonb.data() + value_offset, value_length);
  RETURN_OK();
}
...
Status Jsonb::ScalarToString(const JEntry& element_metadata, const base::span<const char>& json_value,
  ...
  switch (GetJEType(element_metadata)) {
    ...
    case kJEIsArray:
      return ::basis::Status(
        FROM_HERE, ::basis::error::INTERNAL, "Arrays and Objects not supported for this method");
  }
  RETURN_OK();
}
...
Status Jsonb::ApplyJsonbOperator(const base::span<const char>& jsonb, const QLJsonOperationPB& json_op,
  ...
  if (jsonb.size() < sizeof(JsonbHeader)) {
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "Not enough data to process");
  }
  ...
  if ((jsonb_header & kJBScalar) && (jsonb_header & kJBArray)) {
    ...
    return ::basis::Status(
      FROM_HERE, ::basis::error::INTERNAL, "Cannot apply operators to scalar values");
  ...
  return ::basis::Status(
    FROM_HERE, ::basis::error::INTERNAL, "Invalid json operation");
}
