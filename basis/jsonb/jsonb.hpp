...
class Jsonb {
 public:
  ...
  basis::Status FromString(const std::string& json);

  // Creates a serialized jsonb string from json document or value.
  basis::Status FromJson(const base::Value& document);

  // Builds a json document from serialized jsonb.
  basis::StatusOr<base::Value> ToJson() const;

  // Returns a json string for serialized jsonb
  basis::StatusOr<std::string> ToJsonString() const;

#if 0
  basis::Status ToJsonString(std::string* json) const;

  basis::Status ApplyJsonbOperators(const QLJsonColumnOperationsPB& json_ops,
 ...
 private:
  ...
  static basis::Status ApplyJsonbOperator(const base::span<const char>& jsonb, const QLJsonOperationPB& json_op,
  ...
  static basis::Status ToJsonbInternal(const base::Value& document, std::string* jsonb);
  static basis::Status ToJsonbProcessObject(const base::Value& document,
                                             std::string* jsonb);
  static basis::Status ToJsonbProcessArray(const base::Value& document,
                                            bool is_scalar,
                                            std::string* jsonb);

  static basis::Status ProcessJsonValueAndMetadata(const base::Value& document,
  ...
  static basis::Status FromJsonbInternal(const base::span<const char>& jsonb, base::Value* document);

  static basis::Status FromJsonbProcessObject(const base::span<const char>& jsonb,
                                               const JsonbHeader& jsonb_header,
                                               base::Value* document);
  static basis::Status FromJsonbProcessArray(const base::span<const char>& jsonb,
                                              const JsonbHeader& jsonb_header,
                                              base::Value* document);

#if 0
  static basis::Status ToJsonStringInternal(const base::span<const char>& jsonb, std::string* json);
  ...
  static basis::Status CreateScalar(const base::span<const char>& scalar, const JEntry& original_jentry,
                                     std::string* scalar_jsonb);

  // Given a serialized json scalar and its metadata, return a string representation of it.
  static basis::Status ScalarToString(const JEntry& element_metadata, const base::span<const char>& json_value,
                                       std::string* result);
#endif
  ...
  static basis::Status GetArrayElement(size_t index, const base::span<const char>& jsonb,
  ...
  static basis::Status GetObjectKey(size_t index, const base::span<const char>& jsonb, size_t metadata_begin_offset,
  ...
  // metadata information for that element is required.
  static basis::Status GetObjectValue(size_t index, const base::span<const char>& jsonb,
                                       size_t metadata_begin_offset, size_t data_begin_offset,
...
#if 0
  static basis::Status ApplyJsonbOperatorToArray(const base::span<const char>& jsonb,
                                                  const QLJsonOperationPB& json_op,
                                                  const JsonbHeader& jsonb_header,
                                                  base::span<const char>* result,
                                                  JEntry* element_metadata);

  static basis::Status ApplyJsonbOperatorToObject(const base::span<const char>& jsonb,
                                                   const QLJsonOperationPB& json_op,
                                                   const JsonbHeader& jsonb_header,
                                                   base::span<const char>* result,
                                                   JEntry* element_metadata);
