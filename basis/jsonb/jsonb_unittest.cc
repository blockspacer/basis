...
TEST(JsonbTest, TestJsonbSerialization) {
  ...
  base::Optional<base::Value> root(reader.ReadToValue(document));
  EXPECT_TRUE(reader.error_code() == base::JSONReader::JSON_NO_ERROR && root);

  const base::DictionaryValue* dictionary =
      static_cast<const base::DictionaryValue*>(&(*root));
