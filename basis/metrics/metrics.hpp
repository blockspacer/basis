...
//                            "Threads Started",
//                            ::basis::MetricUnit::kThreads,
//                            "Total number of threads started on this server",
//                            ::basis::EXPOSE_AS_COUNTER);
//
//
// Metrics ownership
// ------------------------------------------------------------
...
//
//   METRIC_DEFINE_counter(server, ping_requests, "Ping Requests", ::basis::MetricUnit::kRequests,
...
//   METRIC_DEFINE_counter(my_entity, ping_requests, "Ping Requests", ::basis::MetricUnit::kRequests,
...
#define METRIC_DEFINE_entity(name)                               \
  ::basis::MetricEntityPrototype METRIC_ENTITY_##name(#name)
...
#define METRIC_DEFINE_counter_with_level(entity, name, label, unit, desc, level)   \
  ::basis::CounterPrototype BOOST_PP_CAT(METRIC_, name)(                        \
      ::basis::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity), \
...
#define METRIC_DEFINE_counter(entity, name, label, unit, desc)   \
  METRIC_DEFINE_counter_with_level(entity, name, label, unit, desc, ::basis::MetricLevel::kInfo)
...
#define METRIC_DEFINE_lag_with_level(entity, name, label, desc, level) \
  ::basis::MillisLagPrototype BOOST_PP_CAT(METRIC_, name)( \
      ::basis::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity), \
                                      BOOST_PP_STRINGIZE(name), \
                                      label, \
...
#define METRIC_DEFINE_lag(entity, name, label, desc) \
  METRIC_DEFINE_lag_with_level(entity, name, label, desc, ::basis::MetricLevel::kInfo)

#define METRIC_DEFINE_gauge(type, entity, name, label, unit, desc, level, ...) \
  ::basis::GaugePrototype<type> BOOST_PP_CAT(METRIC_, name)(         \
      ::basis::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity), \
                                      BOOST_PP_STRINGIZE(name), \
                                      label, \
                                      unit, \
...
#define METRIC_DEFINE_gauge_string(entity, name, label, unit, desc, ...) \
    METRIC_DEFINE_gauge(std::string, entity, name, label, unit, desc, \
        ::basis::MetricLevel::kInfo, ## __VA_ARGS__)
...
#define METRIC_DEFINE_histogram_with_percentiles(                              \
  ...
  ::basis::HistogramPrototype BOOST_PP_CAT(METRIC_, name)(                        \
      ::basis::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity),              \
                                      BOOST_PP_STRINGIZE(name), label, unit,   \
                                      desc,                                    \
                                      ::basis::MetricLevel::kInfo),                 \
      max_val, num_sig_digits, ::basis::ExportPercentiles::kTrue)
...
#define METRIC_DEFINE_histogram(entity, name, label, unit, desc, max_val,      \
  ...
  ::basis::HistogramPrototype BOOST_PP_CAT(METRIC_, name)(                        \
      ::basis::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity),              \
                                      ...
                                      desc,                                    \
                                      ::basis::MetricLevel::kInfo),                 \
      max_val, num_sig_digits, ::basis::ExportPercentiles::kFalse)

// The following macros act as forward declarations for entity types and metric prototypes.
#define METRIC_DECLARE_entity(name) \
  extern ::basis::MetricEntityPrototype METRIC_ENTITY_##name
#define METRIC_DECLARE_counter(name)                             \
  extern ::basis::CounterPrototype METRIC_##name
#define METRIC_DECLARE_lag(name) \
  extern ::basis::LagPrototype METRIC_##name
#if TODO
#define METRIC_DECLARE_gauge_string(name) \
  extern ::basis::GaugePrototype<std::string> METRIC_##name
#endif
#define METRIC_DECLARE_gauge_bool(name) \
  extern ::basis::GaugePrototype<bool> METRIC_##name
#define METRIC_DECLARE_gauge_int32(name) \
  extern ::basis::GaugePrototype<int32_t> METRIC_##name
#define METRIC_DECLARE_gauge_uint32(name) \
  extern ::basis::GaugePrototype<uint32_t> METRIC_##name
#define METRIC_DECLARE_gauge_int64(name) \
  extern ::basis::GaugePrototype<int64_t> METRIC_##name
#define METRIC_DECLARE_gauge_uint64(name) \
  extern ::basis::GaugePrototype<uint64_t> METRIC_##name
#define METRIC_DECLARE_gauge_double(name) \
  extern ::basis::GaugePrototype<double> METRIC_##name
#define METRIC_DECLARE_histogram(name) \
  extern ::basis::HistogramPrototype METRIC_##name

#if defined(__APPLE__)
#define METRIC_DEFINE_gauge_size(entity, name, label, unit, desc, ...) \
  ::basis::GaugePrototype<size_t> METRIC_##name(                    \
      ::basis::MetricPrototype::CtorArgs(#entity, #name, label, unit, desc, ## __VA_ARGS__))
#define METRIC_DECLARE_gauge_size(name) \
  extern ::basis::GaugePrototype<size_t> METRIC_##name
#else
...
class MetricEntity : public base::RefCountedThreadSafe<MetricEntity> {
 public:
  ...
  LabelsMap labels() const {
    base::subtle::SpinLock::Guard l(lock_);
    return labels_;
  }
  ...
  basis::Status WriteForPrometheus(PrometheusWriter* writer,
  ...
  size_t num_metrics() const {
    base::subtle::SpinLock::Guard guard(lock_);
 ...
 private:
  ...
  mutable base::subtle::SpinLock lock_;
...
class PrometheusWriter {
 public:
  ...
  basis::Status WriteSingleStringEntry(
  ...
  basis::Status FlushAggregatedValues() {
 ...
 private:
  ...
  virtual basis::Status FlushSingleEntry(const MetricEntity::AttributeMap& attr,
...
#if TODO
  virtual basis::Status FlushSingleStringEntry(const MetricEntity::AttributeMap& attr,
...
class Metric : public base::RefCountedThreadSafe<Metric> {
 public:
  ...
  virtual basis::Status WriteAsJson(
                             const MetricJsonOptions& opts,
  ...
  // Exposes metrics to Prometheus using a simple text-based exposition format.
  // See prometheus.io/docs/instrumenting/exposition_formats/#text-based-format
  virtual basis::Status WriteForPrometheus(
...
class StringGauge : public Gauge {
 public:
  ...
  // Exposes metrics to Prometheus using a simple text-based exposition format.
  // See prometheus.io/docs/instrumenting/exposition_formats/#text-based-format
  basis::Status WriteForPrometheus(
      PrometheusWriter* writer, MetricEntity::AttributeMap& attr,
      const MetricPrometheusOptions& opts,
      int metricsRetirementAgeMs) const override;
 ...
 private:
  std::string value_;
  LabelsMap labels_;
  mutable base::subtle::SpinLock lock_;  // Guards value_
...
class AtomicGauge : public Gauge {
 public:
  ...
  virtual void set_value(const T& value) {
    base::subtle::NoBarrier_Store(&value_, static_cast<int64_t>(value));
  }
  ...
  void Increment() {
    base::subtle::NoBarrier_AtomicIncrement(&value_, 1);
  }
...
class Histogram : public Metric {
 public:
  ...
  basis::Status GetAndResetHistogramSnapshotPB(HistogramSnapshotPB* snapshot,
                                const MetricJsonOptions& opts) const;
#endif
...
//
// // same as
// // basis::ScopedLatencyMetric scoped_processing_duration_hist_metric(
// //   processing_duration_hist_, basis::Auto::kTrue);
//
// SCOPED_LATENCY_METRIC(processing_duration_hist_, basis::Auto::kTrue);
//
#define SCOPED_LATENCY_METRIC(_h, ...) \
  basis::ScopedLatencyMetric scoped##_h##_metric(_h, ## __VA_ARGS__)
...
inline scoped_refptr<Counter> MetricEntity::FindOrCreateCounter(
  ...
  base::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<Counter> m = basis::polymorphic_downcast<Counter*>(FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
    m = new Counter(proto);
...
inline scoped_refptr<MillisLag> MetricEntity::FindOrCreateMillisLag(
  ...
  CheckInstantiation(proto);
  base::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<MillisLag> m = basis::polymorphic_downcast<MillisLag*>(FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
inline scoped_refptr<AtomicMillisLag> MetricEntity::FindOrCreateAtomicMillisLag(
  ...
  CheckInstantiation(proto);
  base::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<AtomicMillisLag> m = basis::polymorphic_downcast<AtomicMillisLag*>(
...
inline scoped_refptr<Histogram> MetricEntity::FindOrCreateHistogram(
  ...
  CheckInstantiation(proto);
  base::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<Histogram> m = basis::polymorphic_downcast<Histogram*>(FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
inline scoped_refptr<Histogram> MetricEntity::FindOrCreateHistogram(
  ...
  base::subtle::SpinLock::Guard l(lock_);
  auto m = basis::polymorphic_downcast<Histogram*>(FindPtrOrNull(metric_map_, proto.get()).get());
  if (!m) {
...
inline scoped_refptr<AtomicGauge<T> > MetricEntity::FindOrCreateGauge(
  ...
  CheckInstantiation(proto);
  base::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<AtomicGauge<T> > m = basis::polymorphic_downcast<AtomicGauge<T>*>(
      FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
inline scoped_refptr<AtomicGauge<T> > MetricEntity::FindOrCreateGauge(
  ...
  CheckInstantiation(proto.get());
  base::subtle::SpinLock::Guard l(lock_);
  auto m = basis::polymorphic_downcast<AtomicGauge<T>*>(FindPtrOrNull(metric_map_, proto.get()).get());
  if (!m) {
...
inline scoped_refptr<FunctionGauge<T> > MetricEntity::FindOrCreateFunctionGauge(
  ...
  CheckInstantiation(proto);
  base::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<FunctionGauge<T> > m = basis::polymorphic_downcast<FunctionGauge<T>*>(
      FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
inline scoped_refptr<StringGauge > MetricEntity::FindOrCreateStringGauge(
  ...
  CheckInstantiation(proto);
  base::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<StringGauge > m = basis::polymorphic_downcast<StringGauge*>(
      FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
