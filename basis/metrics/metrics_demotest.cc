...
METRIC_DEFINE_gauge_string(test_entity, fake_memory_usage, "Memory Usage",
                           basis::MetricUnit::kBytes, "Test Gauge 1");
#endif

METRIC_DEFINE_gauge_int64(test_entity, test_func_gauge, "Test Gauge",
                          basis::MetricUnit::kBytes, "Test Gauge 2");
...
public:
    ...
    http_worker(tcp::acceptor& acceptor, const std::string& doc_root,
                basis::MetricRegistry& registry, scoped_refptr<basis::MetricEntity> entity) :
        acceptor_(acceptor),
        doc_root_(doc_root),
        registry_(registry),
        entity_(entity)
...
private:
    ...
    basis::MetricRegistry& registry_;

    scoped_refptr<basis::MetricEntity> entity_;
    ...
    scoped_refptr<basis::Counter> connections_created_
      = METRIC_http_requests_created.Instantiate(entity_);

    scoped_refptr<basis::Histogram> processing_duration_hist_
      = METRIC_http_request_processing_duration.Instantiate(entity_);

    scoped_refptr<basis::AtomicGauge<int64_t> > test_func_usage =
      METRIC_test_func_gauge.Instantiate(entity_, 0);

#if TODO
    scoped_refptr<basis::StringGauge > mem_usage =
