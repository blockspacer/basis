...
#include "basis/race_sync_point/race_sync_point.hpp"
...
basis::Status checkCyclesInHierarchy(
  const std::vector<RaceSyncPoint::SyncDependency>& dependencies)
{
  ...
  for (const auto& dependency : dependencies) {
    scoped_refptr<Dependency> successorContents
      = base::LookupOrInsert(&dependencyHierarchy
...
#if TODO
int basis_kill_odds = 0;

std::vector<std::string> basis_kill_prefix_blacklist;

void TestKillRandom(std::string kill_point, int odds,
                    const std::string& srcfile, int srcline) {
  for (auto& p : basis_kill_prefix_blacklist) {
...
basis::Status RaceSyncPoint::ClearAndLoadDependencies(
