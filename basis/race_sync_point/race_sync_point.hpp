...
#define TEST_KILL_RANDOM(kill_point, basis_kill_odds)
#else
...
extern std::vector<std::string> basis_kill_prefix_blacklist;
...
#define TEST_KILL_RANDOM(kill_point, basis_kill_odds)                  \
  {                                                                      \
    if (basis_kill_odds > 0) {                                         \
      TestKillRandom(kill_point, basis_kill_odds, __FILE__, __LINE__); \
    }                                                                    \
  }
...
class RaceSyncPoint {
 public:
  ...
  // sync points
  //
  basis::Status ClearAndLoadDependencies(
...
//
#define TEST_SYNC_POINT(x) basis::RaceSyncPoint::GetInstance()->Process(SyncSuccessor{x})
#define TEST_SYNC_POINT_CALLBACK(x, y) \
  basis::RaceSyncPoint::GetInstance()->Process(SyncSuccessor{x}, (void*)y)
#endif  // NDEBUG
