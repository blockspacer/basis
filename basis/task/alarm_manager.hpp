...
/// \note Prefer `AlarmManager` to `PostDelayedTask`
/// if you need to run task at specific time.
//
...
class AlarmManager {
 public:
  static constexpr int kDefaultClockPollSec = 5;
  ...
  // Allows setting a fake clock and using a custom task runner during testing.
  AlarmManager(std::unique_ptr<base::Clock> clock
               , scoped_refptr<base::SingleThreadTaskRunner> task_runner
               , const base::TimeDelta& polling_frequency
                   = base::TimeDelta::FromSeconds(kDefaultClockPollSec));
