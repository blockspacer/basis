...
#include "basis/varint/fast_varint.hpp"
#include "basis/varint/varint.hpp"
...
basis::StatusOr<int64_t> Test_FastDecodeUnsignedVarIntFromBytesStr(base::StringPiece piece)
{
...
basis::StatusOr<int64_t> Test_FastDecodeDescendingSignedVarInt(base::StringPiece piece)
{
  RETURN_ERROR_IF(piece.empty())
    << "Piece not fully decoded.";
  const uint8_t* data = fast_varint_detail::to_uchar_ptr(piece.data());
...
TEST(FastVarIntTest, Unsigned) {
  ...
  for (size_t i = 0; i != kTotalValues; ++i) {
    basis::StatusOr<uint64_t> decoded_value = FastDecodeUnsignedVarInt(&encoded_values[i]);
...
TEST(FastVarIntTest, DecodeUnsignedEmptyNullTerminated) {
  ...
  basis::StatusOr<uint64_t> result = Test_FastDecodeUnsignedVarIntFromBytesStr(value);
  EXPECT_OK(result)
    << "Input: " << value;
...
TEST(FastVarIntTest, DecodeDescendingSignedCheck) {
  ...
  for (auto value : values) {
    SCOPED_TRACE(base::Substitute("Value: $1", value));
