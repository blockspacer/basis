...
#include "basis/varint/varint.hpp" // IWYU pragma: associated
#include "basis/core/pointer_cast.hpp"
...
basis::StatusOr<int64_t> VarInt::ToInt64() const {
  ...
  //   << base::Substitute("VarInt $1 cannot be converted to int64"
  ...
  CHECK_EQ(expected_without_overflow, !has_overflow)
    << base::Substitute("VarInt $1 has wrong overflow check. "
                        "Because limit is $2."
                        , *this, limit);
#endif // DCHECK_IS_ON()
  ...
  CHECK_EQ(expected_output, detected_output)
    << base::Substitute("VarInt $1 has wrong value after type conversions. "
...
basis::Status VarInt::FromString(const char* cstr) {
  ...
  RETURN_ERROR_IF(parsed == 0 || parsed != cstr_len, INVALID_ARGUMENT)
    << base::Substitute("Cannot parse varint: $1", cstr);
  RETURN_OK();
}
...
std::string VarInt::EncodeToComparable(size_t num_reserved_bits) const {
  ...
  auto data = basis::pointer_cast<unsigned char*>(const_cast<char*>(result.data()));
...
basis::Status VarInt::DecodeFromComparable(const base::StringPiece& slice, size_t *num_decoded_bytes,
  ...
  while (buffer[idx] == 0xff) {
    ...
    RETURN_ERROR_IF(idx >= len)
      << base::Substitute("Encoded varint failure, no prefix termination: $1", base::HexEncode(slice.data(), slice.size()));
    num_ones += 8;
  }
  uint8_t temp = 0x80;
  ...
  RETURN_ERROR_IF(num_ones > len)
    << base::Substitute("Not enough data in encoded varint: $1, $2",
...
basis::Status VarInt::DecodeFromComparable(const base::StringPiece& slice) {
  size_t num_decoded_bytes;
  return DecodeFromComparable(slice, &num_decoded_bytes);
}

basis::Status VarInt::DecodeFromComparable(const std::string& str) {
...
basis::Status VarInt::DecodeFromTwosComplement(const std::string& input) {
  using namespace ::basis::error;
  ...
  if (!negative) {
    impl_.reset(BN_bin2bn(
        basis::pointer_cast<const unsigned char*>(input.data()), input.size(), nullptr /* ret */));
    RETURN_OK();
  }
  std::string copy(input);
  auto data = basis::pointer_cast<unsigned char*>(const_cast<char*>(copy.data()));
