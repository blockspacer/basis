...
#include "basis/strings/string_util.hpp"
...
class VarInt {
 public:
  ...
  basis::StatusOr<int64_t> ToInt64() const;

  // The input is expected to be of the form (-)?[0-9]+, whitespace is not allowed. Use this
  // after removing whitespace.
  basis::Status FromString(const std::string& input) {
    return FromString(input.c_str());
  }

  basis::Status FromString(const char* cstr);

  static basis::StatusOr<VarInt> CreateFromString(const std::string& input) {
  ...
  static basis::StatusOr<VarInt> CreateFromString(const char(&input)[N]) {
    using namespace basis::error;
    VarInt result;
  ...
  basis::Status DecodeFromComparable(
  ...
  basis::Status DecodeFromComparable(const base::StringPiece& string);
  basis::Status DecodeFromComparable(const std::string& string);
  ...
  basis::Status DecodeFromTwosComplement(const std::string& string);
