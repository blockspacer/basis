 ...
 protected:
  void SetUp() override {
    ...
    for (const auto& input : inputs) {
      basis::StatusOr<VarInt> status = VarInt::CreateFromString(input);
      ASSERT_OK(status);
...
TEST_F(VarIntTest, TestComparableEncoding) {
  ...
  for (size_t i = 0; i != values_.size(); ++i) {
    SCOPED_TRACE(base::Substitute("Index: $1, value: $2", i, values_[i]));
...
TEST_F(VarIntTest, TestTypeOverflowLarge) {
  basis::StatusOr<VarInt> status = VarInt::CreateFromString("9223372036854775808");
  ASSERT_OK(status);
...
TEST_F(VarIntTest, TestTypeOverflowSmall) {
  basis::StatusOr<VarInt> status = VarInt::CreateFromString("-9223372036854775809");
  ASSERT_OK(status);
...
TEST_F(VarIntTest, TestTypeOverflowLarger) {
  basis::StatusOr<VarInt> status = VarInt::CreateFromString("99999999999999999999999");
  ASSERT_OK(status);
...
TEST_F(VarIntTest, ComparableEncodingWithReserve) {
  ...
  for (int i = 0; i != 0xffff; ++i) {
    SCOPED_TRACE(base::Substitute("i: $1", i));
    VarInt v(i);
