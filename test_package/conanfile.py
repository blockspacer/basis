...
class TestPackageConan(conan_build_helper.CMakePackage):
    ...
    def build_requirements(self):
        ...
        if self.options['basis'].enable_tsan \
            or self.options['basis'].enable_msan \
            or self.options['basis'].enable_asan \
            or self.options['basis'].enable_ubsan:
    ...
    def build(self):
        ...
        cmake.definitions['ENABLE_UBSAN'] = self.options['basis'].enable_ubsan
        cmake.definitions['ENABLE_ASAN'] = self.options['basis'].enable_asan
        cmake.definitions['ENABLE_MSAN'] = self.options['basis'].enable_msan
        cmake.definitions['ENABLE_TSAN'] = self.options['basis'].enable_tsan
    ...
    def test(self):
        if not tools.cross_building(self):
            ...
            bin_path = os.path.join(self.build_folder, "basis_test_package")
